...
class GLU(gluon.nn.Block):
  ...
  def forward(self, x, last_outputs, pixel_shuffle=False, ps_layer=None):
    gate = self.Conv(x)
    if pixel_shuffle==True:
      gate = ps_layer(gate)
    outputs = gate * last_outputs
    return outputs
...
class PixelShuffler(gluon.nn.Block):
  ...
  def forward(self, x, size=(1, 2)):
    ...
    oh, ow = h * rh, w * rw
    oc = c // (rh * rw)
    outputs = nd.reshape(data=x, shape=(n, rh, rw, oc, h, w))
    outputs = nd.moveaxis(tensor=outputs, source=2, destination=-1)
    outputs = nd.moveaxis(tensor=outputs, source=1, destination=-3)
...
class Generator(gluon.nn.Block):
  ...
  def __init__(self, **kwargs):
    ...
    self.GLU_in = GLU(channels=128, kernel_size=(1, 15), strides=(1, 1), padding=(0, 7))

    self.Conv_d1 = gluon.nn.Conv2D(channels=256, kernel_size=(1, 5), strides=(1, 2), padding=(0, 2))
    self.IN_d1 = InstanceNorm(channels=256)
    self.GLU_d1 = GLU(channels=256, kernel_size=(1, 5), strides=(1, 2), padding=(0, 2))
    self.Conv_d2 = gluon.nn.Conv2D(channels=512, kernel_size=(1, 5), strides=(1, 2), padding=(0, 2))
    self.IN_d2 = InstanceNorm(channels=512)
    self.GLU_d2 = GLU(channels=512, kernel_size=(1, 5), strides=(1, 2), padding=(0, 2))
    ...
    self.GLU_r1_1 = GLU(channels=1024, kernel_size=(1, 3), strides=(1, 1), padding=(0, 1))
    ...
    self.GLU_r2_1 = GLU(channels=1024, kernel_size=(1, 3), strides=(1, 1), padding=(0, 1))
    ...
    self.GLU_r3_1 = GLU(channels=1024, kernel_size=(1, 3), strides=(1, 1), padding=(0, 1))
    ...
    self.GLU_r4_1 = GLU(channels=1024, kernel_size=(1, 3), strides=(1, 1), padding=(0, 1))
    ...
    self.GLU_r5_1 = GLU(channels=1024, kernel_size=(1, 3), strides=(1, 1), padding=(0, 1))
    ...
    self.GLU_r6_1 = GLU(channels=1024, kernel_size=(1, 3), strides=(1, 1), padding=(0, 1))
  ...
  def forward(self, inputs):
    # data.shape = 'NCHW' h1-wT-c24
    # Preprocess
    inputs_tmp = self.Conv_in(inputs)
    inputs = self.GLU_in(inputs, inputs_tmp)

    # Downsample
    downsample1 = self.Conv_d1(inputs)
    downsample1 = self.IN_d1(downsample1)
    downsample1 = self.GLU_d1(inputs, downsample1)
    downsample2 = self.Conv_d2(downsample1)
    downsample2 = self.IN_d2(downsample2)
    downsample2 = self.GLU_d2(downsample1, downsample2)
    ...
    residual_1 = self.GLU_r1_1(residual_input, residual_1)
    ...
    residual_2 = self.GLU_r2_1(residual_1, residual_2)
    ...
    residual_3 = self.GLU_r3_1(residual_2, residual_3)
    ...
    residual_4 = self.GLU_r4_1(residual_3, residual_4)
    ...
    residual_5 = self.GLU_r5_1(residual_4, residual_5)
    ...
    residual_6 = self.GLU_r6_1(residual_5, residual_6)
